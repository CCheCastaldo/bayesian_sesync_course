...
Let's plot each covariate to get a sense of how their values are distributed spatially.
...
Let's plot the semivariance. Remember that the semivariance is:

$$\gamma(d) = \text{cov}(0) - \text{cov}(d) + \sigma^{2}_{\epsilon}$$

```{r, echo = TRUE, fig.align = 'center'}
distance_plot = seq(0, limit, 1)
y.sig = sigma.s - sigma.s * (exp(-distance_plot / phi))
...
We need to examine the possibility for spatial autocorrelation in the residuals by plotting a semivariogram, a plot of the semivarince as a function of distance between points. We will use the `variogram` function from the `gstats` package to do this. Remember that the y-axis is gamma:
...
&\times \text{normal}(\boldsymbol{\eta}\mid\mathbf{0},\boldsymbol{\Sigma})\prod_{j=1}^3\text{normal}(\beta_{j} \mid 0,10000)\text{uniform}(\sigma^2_{strct}\mid0,100)\\
...
2. You can create a zero centered random variable reflecting spatial structure using code such as `eta ~ dmnorm(mu_eta, inverse(Cov.mat))` where `mu.eta` is  a vector of zeros whose length equals number of rows in `Cov.mat`.
...
5. Spatial models tend to run slowly the large covariance matrix is large. Moreover, fitting these models requires many data points, so we can't speed the process by making a small, simulated data set. We have provided the R code to allow you to run your model in parallel.  All you need to do is write the JAGS code for the model. You may omit posterior predictive checks to reduce execution time.
...
model {
  ...
  prec.mat = inverse(sigma.u^2 * y.I) 
...
initsP = list(
  list(beta = c(8, 7, 3), sigma.s = 2.5, sigma.u = 2.8, phi = 83,
    RNG.seed = 1, .RNG.name = "base::Mersenne-Twister"),
  list(beta = c(3, 2, 1), sigma.s = 1.5, sigma.u = 3.8, phi = 43,
    RNG.seed = 11, .RNG.name = "base::Mersenne-Twister"),
  list(beta = c(11, 6, 9), sigma.s = 1, sigma.u = 2, phi = 23,
...
out <- clusterEvalQ(cl, {
  ...
  zm = coda.samples(jm, variable.names = c("beta", "e", "sigma.u", "sigma.s", "phi"), 
...
Produce new output from your spatially structured model.  Compare it with the output from the apspatial model. What is different about the marginal posteriors?  Examine evidence for autocorrelation with a semivariogram.  What do you conclude?
...
plot(eps_v2$dist, eps_v2$gamma, ylim = c(0, 3), xlab = "distance between obs i and j", ylab = "gamma")
```

The standard deviations of marginal posterior distributions of parameters approximated using the spatially unstructred model are about half of those approximated using the spatially structured model. The semivariogram indicates no spatial depdence after the spatially strcutured random effects are removed from the residuals.
