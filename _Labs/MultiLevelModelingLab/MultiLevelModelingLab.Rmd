...
2. Draw a Bayesian network and write out the posterior and joint distribution for a linear regression model of N2O emission ($y_{i}$
) on fertilizer addition ($x_{i}$) for a single observation.
...
3. Write out the joint distribution for a linear regression model of N~2~O emission ($y$) on fertilizer addition ($x$) for a single observation. Start by using generic $[\,]$. Use $\sigma^{2}$ to represent the uncertainty in your model realizing, of course, that you might need moment matching when you choose a specific distribution.  
...
1. Draw a Bayesian network and write out the posterior and joint distribution for a linear regression model of N~2~O emission ($y_{ij}$) on fertilizer addition ($x_{ij}$) for a single observation. 
...
$$
\big[\boldsymbol{\alpha},\beta,\sigma \mid y_{ij}\big] \propto  \big[\log(y_{ij})\mid g\big(\alpha_{j},\beta,\log(x_{ij})\big),\sigma^{2}\big][\boldsymbol{\alpha}]\big[\beta\big]\big[\sigma \big]
$$
...
$$
\begin{align*}
[\boldsymbol{\alpha},\beta,\sigma \mid \mathbf{y}]&\propto\prod_{i=1}^{n}\prod_{j=1}^{J}\text{normal}\big(\log(y_{ij})\mid g\big(\alpha_{j},\beta,\log(x_{ij}),\sigma^2\big)\\
...
$$
\begin{align*}
[\boldsymbol{\alpha},\beta,\sigma,\mu_\alpha,\varsigma_\alpha \mid \mathbf{y}]&\propto \prod_{i=1}^{n_j} \prod_{j=1}^{J}\text{normal}(\log(y_{ij})\mid g\big(\alpha_j,\beta,\log(x_{ij}),\sigma^2\big)\\
...
<button class="button" onclick="toggle_visibility('myDIV17A');">Answer</button>

<div id="myDIV17A", style="display:none">
...
8. Draw a Bayesian network and write out the posterior and joint distribution for a linear regression model of N~2~O emission ($y_{ij}$) on fertilizer addition ($x_{ij}$) for a single observation. 
...
9. Write out the posterior and joint distributions for the full dataset, which means you must include the proper products. Use $ij$ notation. You will need to notate that there are $n_{j}$ measurements of N~2~O emissions paired with fertilizer additions from site $j$. Choose appropriate distributions for each random variable. 
...
<button class="button" onclick="toggle_visibility('myDIV22');">Answer</button>
...
2. Write out the posterior and joint distributions for the full dataset, which means you must include the proper products. Choose appropriate distributions for each random variable. 

<button class="button" onclick="toggle_visibility('myDIV23');">Answer</button>
...
<button class="button" onclick="toggle_visibility('myDIV24');">Answer</button>
...
model{
  ...
  sigma.alpha ~ dunif(0,200) #notated as varsigma in model documentation
  tau.alpha <- 1 / sigma.alpha^2
  sigma.beta ~ dunif(0,200) #notated as varsigma in model documentation
...
<button class="button" onclick="toggle_visibility('myDIV25');">Answer</button>
...
5. Draw a Bayesian network and write out the posterior and joint distribution for a linear regression model of N~2~O emission ($y_{ij}$) on fertilizer addition ($x_{ij}$) for a single observation. 

<button class="button" onclick="toggle_visibility('myDIV26');">Answer</button>
...
6. Write out the posterior and joint distributions for the full dataset, which means you must include the proper products. Choose appropriate distributions for each random variable. 

<button class="button" onclick="toggle_visibility('myDIV27');">Answer</button>
...
<button class="button" onclick="toggle_visibility('myDIV28');">Answer</button>
...
jm.pooled = jags.model(file="PooledJAGS.R", data = data, n.adapt = n.adapt, inits = inits, n.chains = length(inits))
update(jm.pooled, n.iter = n.update)
zc.pooled = coda.samples(jm.pooled, variable.names = c("p.sd", "p.mean", "p.discrep", "p.min", "log.emission.sim"), 
  n.iter = n.iter)
```
...
lines(density(MCMCchains(zc.pooled, params = "log.emission.sim")), col = "red")
