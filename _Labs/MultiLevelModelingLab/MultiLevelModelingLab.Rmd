...
$$
\big[\boldsymbol{\alpha},\beta,\sigma \mid y_{i}\big] \propto  \big[\log(y_{ij})\mid g\big(\alpha_{j},\beta,\log(x_{ij})\big),\sigma^{2}\big][\boldsymbol{\alpha}]\big[\beta\big]\big[\sigma \big]
$$
...
$$
\begin{align*}
[\boldsymbol{\alpha},\beta,\sigma \mid \mathbf{y}]&\propto\prod_{i=1}^{n}\text{normal}\big(\log(y_{ij})\mid g\big(\alpha_{j},\beta,\log(x_{ij}),\sigma^2\big)\\
