...
&\times [\beta_{0}][\beta_{1}][\beta_{2}][\mu][\varsigma^{2}]
\end{align*}$$

An colleague objects to your assumption of cover observed perfectly by eye, insisting, reasonably we think, that you develop a data model relating your ocular estimate to the true cover in a plot. So, you obtained visual estimates of cover paired with the actual proportion of vegetated area (measured using small sub-plots) on 15 10 x 10 m plots. After days of sweaty labor, you regressed visual estimates $(x_{i})$ on the true cover $(z_{i})$ and developed a calibration equation $h(\bm{\alpha},z_{i})$:
...
\prod_{k=1}^{15}\text{beta}\bigg(x_{ik} \mid m\bigg(\frac{e^{\eta_{0} + \eta_{1}z_{i}}}{e^{\eta_{0} + \eta_{1}z_{i}}+1}, \sigma^{2}\bigg)\bigg)\\ &\times [\beta_{0}][\beta_{1}][\beta_{2}][\mu][\sigma^{2}]
\text{normal}(\eta_{0}\mid 0.05, 0.006)\text{normal}(\eta_{1} \mid 1.07, 0.13)\text{inverse gamma}(\varsigma^{2} \mid 10.2, 630)
\end{align*}$$

$$\begin{align*}
h(\pmb{\alpha},z_{i})  = &\, \frac{e^{\alpha_{o}+\alpha_{1}z_{i}}}{1+e^{\alpha_{o}+\alpha_{1}z_{i}}}\\
x_{i}  \sim &\, \text{beta}\big(m(h(\pmb{\alpha},z_{i}),\varsigma^{2})\big)\\
\alpha_{o}  \sim &\, \text{\text{normal}(.05,.006)}\\
\alpha_{1}  \sim &\, \text{normal}(1.07,.13)\\
\varsigma^{2}  \sim &\, \text{inverse gamma}(10.2,630)
\end{align*}$$

The function $m(\,)$ returns parameters of the beta distribution given moments as inputs. Include the calibration equation in your model of effects of soil water and herbaceous cover on seedling establishment using informed priors on $\alpha_{0},\alpha_{1}$ and $\varsigma^{2}$.Hint: think about the predictor variable for herbaceous cover. Do you want to use the observed value of cover $(x_{i})$ or the true value $(z_{i})$ to model its effect on establishment?
