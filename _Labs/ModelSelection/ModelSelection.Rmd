...
Specifying linear models in matrix notation is compact and convenient in comparison to writing out the full model as a scalar equation in the context of several predictor variables.  Consider a typical deterministic linear model with three coefficients:
...
where $\pmb{\beta}$ is a column vector $(\beta_0, \beta_1,\beta_3)$ with length = number of model coefficients, and $\mathbf{X}$ is a *design* matrix with the number of rows equal to the number of data points and the number of columns equal to the number of predictor variables + 1.  The first column contains all 1's (or 0's if you seek to force the intercept through 0).  The second column contains the data values for predictor variable 1,  the third column contains the data values for predictor variable 2, and so on. $\mathbf(y)$ is a vector of model output containing a prediction for each data point.  See the lecture notes covering matrix multiplication if needed.

Statisticians use matrix notation exclusively to specify linear models, although notation is not consistent. It's beneficial to become comfortable with matrix notation. This is particularly handy here because we can use a single JAGS file (well, two actually) to specify several different models using the code below. 
...
1. Write an expression for the posterior and joint distribution for each of the four models.
