...
n.update = 5000
n.iter = 5000
jm.pooled = jags.model(file="RandomInterceptJAGS.R", data = data, n.adapt = n.adapt, inits = inits, n.chains = length(inits))
update(jm.pooled, n.iter = n.update)
zc.randomint = coda.samples(jm.pooled, variable.names = c("alpha", "beta", "sigma", "mu.alpha", "sigma.alpha", 
  "mu_pred", "log_mu_pred"), n.iter = n.iter)
```

```{r, echo = TRUE, fig.align = 'center', fig.width = 10, fig.height = 4}
MCMCplot(zc.randomint, params = c("alpha", "mu.alpha"), sz_labels = .4, sz_med = .75, sz_thin = .7, sz_thick = 2, horiz = FALSE)
```
