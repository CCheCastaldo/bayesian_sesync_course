...
- Be able to write models for different types of data.
...
Recall that the posterior distribution is proportional to their joint distribution:
$$[\theta|y]\propto[\theta,y].$$

The joint distribution can be factored into a likelihood and priors:

$$\big[\theta, \sigma^{2}\big]= \big[y \mid \theta, \sigma^{2}\big]\big[\theta\big]\big[\sigma^{2}\big]$$
We often wish to model the mean response to $y$. To do this we embed some ecological or socio-environmental deterministic function $g\big(\boldsymbol{\beta},x\big)$ into the likelihood like this....

$$\big[\boldsymbol{\theta}, \sigma^{2}\big]\propto \big[y \mid g\big(\boldsymbol{\beta},x\big), \sigma^{2}\big]\big[\boldsymbol{\beta}\big]\big[\sigma^{2}\big]$$

## A simple example with an old friend

\centerline{
\includegraphics[width=.5\textwidth]{../../_Graphics/data.pdf}
}
\vspace{-1cm}

\begin{align*}
g\left(\boldsymbol{\beta,}x_{i}\right) = & \beta_{0}+\beta_{1}x_{i}\\
\left[\boldsymbol{\beta},\sigma^{2}|\boldsymbol{y}\right] \propto & \prod_{i=1}^{15}\textrm{normal}\left(y_{i}|g\left(\boldsymbol{\beta,}x_{i}\right),\sigma^{2}\right)\textrm{normal}\left(\beta_{0}|0,1000\right)\times\\
 &  \textrm{normal}\left(\beta_{1}|0,1000\right)\times\\
 &  \textrm{inverse gamma}\left(\sigma^{2}|0.001,0.001\right)\\
\end{align*}
...
\centerline{
\includegraphics[width=.6\textwidth]{../../_Graphics/DeterministicModelsAndTypesOfData.png}
}
...
- Moment matching (which you are familiar with)
- Mapping our deterministic model $g\big(\boldsymbol{\beta},x\big) = \beta_{0} + ... + \beta_{n}x_{n}$ from $(-\infty, \infty)$ to a range appropriate for the likelihood in our model
